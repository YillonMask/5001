...
def start_menu():
    ...
    while run:
        ...
        instruction_text = font.render(
            "Press i for Instructions & Credits", True, (255, 255, 255)
        )
        exit_text = font.render("Press ESC to Exit", True, (255, 255, 255))
        gameDisplay.blit(
            title_text,
            (
                display_width / 2 - title_text.get_width() / 2,
...
def instruction():
    ...
    while run:
        ...
        return_text = font.render(
            "Press R to return to the start menu", True, (255, 255, 255)
        )
        credit_text_1 = credit_font.render(
            "Northeastern University CS 5001 Final Project", True, (255, 255, 255)
        )
        credit_text_2 = credit_font.render(
            "Created by: Byunghyun Ko, Deo Gracias Ong, Xinrui Yi",
            True,
            (255, 255, 255),
        )
        instruction_link = font.render(
            "Press HERE for instructions", True, (255, 255, 255)
        )
        gameDisplay.blit(
            title_text,
            (
        ...
        gameDisplay.blit(
            return_text,
            (
                display_width / 2 - return_text.get_width() / 2,
        ...
        gameDisplay.blit(
            credit_text_1,
            (
                display_width / 2 - credit_text_1.get_width() / 2,
        ...
        gameDisplay.blit(
            credit_text_2,
            (
                display_width / 2 - credit_text_2.get_width() / 2,
                display_height / 1.1 - credit_text_2.get_height() / 2,
            ),
        )
        instruction_link = gameDisplay.blit(
            instruction_link,
            (
...
def end_of_round_menu(win_status, bank, wins, losses, rounds, pushes):
    ...
    while run:
        ...
        title_text = title_font.render(
            "YOU " + win_status + " THIS ROUND.", True, (255, 0, 0)
        )
        deal_text = font.render("Press SPACE to Deal Again", True, (255, 255, 255))
        instruction_text = font.render(
            "Press i for Instructions", True, (255, 255, 255)
        )
        exit_text = exit_font.render("Press ESC to Exit", True, (255, 255, 255))

        bank_text = statistics_font.render(
            "Total Gains or Losses: " + str(bank) + " USD", True, (255, 255, 255)
        )
        rounds_text = statistics_font.render(
            "Rounds Played: " + str(rounds), True, (255, 255, 255)
        )
        wins_text = statistics_font.render("Wins: " + str(wins), True, (255, 255, 255))
        losses_text = statistics_font.render(
            "Losses: " + str(losses), True, (255, 255, 255)
        )
        pushes_text = statistics_font.render(
